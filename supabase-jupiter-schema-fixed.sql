...
-- Fixed PostgreSQL syntax - Run this SQL in your Supabase SQL editor
...
-- ============================================================================
-- INDEXES (created separately from tables)
-- ============================================================================

-- Indexes for jupiter_swaps
CREATE INDEX IF NOT EXISTS idx_jupiter_swaps_user_wallet ON jupiter_swaps(user_wallet);
CREATE INDEX IF NOT EXISTS idx_jupiter_swaps_creator_wallet ON jupiter_swaps(creator_wallet);
CREATE INDEX IF NOT EXISTS idx_jupiter_swaps_post_id ON jupiter_swaps(post_id);
CREATE INDEX IF NOT EXISTS idx_jupiter_swaps_signature ON jupiter_swaps(signature);
CREATE INDEX IF NOT EXISTS idx_jupiter_swaps_created_at ON jupiter_swaps(created_at DESC);

-- Indexes for jupiter_quotes
CREATE INDEX IF NOT EXISTS idx_jupiter_quotes_request_id ON jupiter_quotes(request_id);
CREATE INDEX IF NOT EXISTS idx_jupiter_quotes_user_wallet ON jupiter_quotes(user_wallet);
CREATE INDEX IF NOT EXISTS idx_jupiter_quotes_executed ON jupiter_quotes(executed);
CREATE INDEX IF NOT EXISTS idx_jupiter_quotes_created_at ON jupiter_quotes(created_at DESC);

-- Indexes for jupiter_fee_distributions
CREATE INDEX IF NOT EXISTS idx_jupiter_fee_distributions_creator ON jupiter_fee_distributions(creator_wallet);
CREATE INDEX IF NOT EXISTS idx_jupiter_fee_distributions_pending ON jupiter_fee_distributions(pending_distribution DESC);

-- Indexes for jupiter_fee_claims
CREATE INDEX IF NOT EXISTS idx_jupiter_fee_claims_creator ON jupiter_fee_claims(creator_wallet);
CREATE INDEX IF NOT EXISTS idx_jupiter_fee_claims_signature ON jupiter_fee_claims(signature);
CREATE INDEX IF NOT EXISTS idx_jupiter_fee_claims_status ON jupiter_fee_claims(status);
CREATE INDEX IF NOT EXISTS idx_jupiter_fee_claims_created_at ON jupiter_fee_claims(created_at DESC);
...
DROP TRIGGER IF EXISTS trigger_update_fee_distribution ON jupiter_swaps;
...
DROP TRIGGER IF EXISTS trigger_mark_quote_executed ON jupiter_swaps;
...
DROP POLICY IF EXISTS "Users can view their own swaps" ON jupiter_swaps;
DROP POLICY IF EXISTS "Creators can view swaps for their tokens" ON jupiter_swaps;
DROP POLICY IF EXISTS "Service role can manage all swaps" ON jupiter_swaps;
DROP POLICY IF EXISTS "Allow insert for authenticated users" ON jupiter_swaps;

DROP POLICY IF EXISTS "Users can view their own quotes" ON jupiter_quotes;
DROP POLICY IF EXISTS "Service role can manage all quotes" ON jupiter_quotes;
DROP POLICY IF EXISTS "Allow insert for authenticated users" ON jupiter_quotes;

DROP POLICY IF EXISTS "Creators can view their own fee distributions" ON jupiter_fee_distributions;
DROP POLICY IF EXISTS "Service role can manage all fee distributions" ON jupiter_fee_distributions;

DROP POLICY IF EXISTS "Creators can view their own fee claims" ON jupiter_fee_claims;
DROP POLICY IF EXISTS "Service role can manage all fee claims" ON jupiter_fee_claims;
...
CREATE POLICY "Allow insert for authenticated users"
  ON jupiter_swaps FOR INSERT
  WITH CHECK (true);

-- RLS Policies for jupiter_quotes
...
DO $$
BEGIN
  RAISE NOTICE '✅ Jupiter schema migration completed successfully!';
  RAISE NOTICE '📊 Created 4 tables: jupiter_swaps, jupiter_quotes, jupiter_fee_distributions, jupiter_fee_claims';
  RAISE NOTICE '📈 Created 1 view: creator_earnings_dashboard';
